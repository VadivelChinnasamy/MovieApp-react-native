...
class DetailScreen extends Component {
  ...
  getMovieDetail(itemId) {
    axios
      .get(config.movies.movieDetail + itemId + config.movies.apiKey)
      .then(res => {
        this.setState({
          response: res.data
        });
    ...
    console.log("DETAIL PAGE : " + this.state.response.overview);
  }
    ...
    return (
      <ScrollView style={styles.container}>
        ...
        <View style={[styles.container, styles.paddingValue]}>
          ...
          <FlatList
            data={this.state.review.results}
            renderItem={({ item }) => (
              <View style={pageStyle.reviewView}>
                <Text style={pageStyle.reviewAuthor}>{item.author}</Text>

                <Text style={pageStyle.reviewComment}>{item.content}</Text>
              </View>
            )}
          />
        </View>
